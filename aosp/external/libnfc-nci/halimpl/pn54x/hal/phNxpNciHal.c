...
int phNxpNciHal_open(nfc_stack_callback_t *p_cback, nfc_stack_data_callback_t *p_data_cback)
{
    ...
    static uint8_t cmd_init_nci[] = {0x20,0x01,0x02, 0x01, 0x00};
#else
...
#if(NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    isfound =  GetNxpByteArrayValue(NAME_NXP_NFC_MERGE_RF_PARAMS,(char *)buffer,bufflen,&retlen);
    if(retlen > 0)
    {
        status = phNxpNciHal_send_ext_cmd(retlen,buffer);
        if (status != NFCSTATUS_SUCCESS)
        {
            NXPLOG_NCIHAL_E("NFCC Configuration Control set config failed");
        }
    }
#endif
