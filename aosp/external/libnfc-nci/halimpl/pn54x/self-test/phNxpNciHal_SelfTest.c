...
#define HAL_WRITE_RSP_TIMEOUT   (1000)   /* Timeout value to wait for response from PN54X */
    ...
    {
        {
            ...
            0x05, {0x20,0x01,0x02,0x01,0x00} /* cmd */
#else
        ...
        {
#if(NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
            0x4, {0x40,0x01,0x1B,0x00 } /* exp_rsp */
#else
...
/* for rf field test, first reuires to disable the standby mode */
        ...
        uint8_t tech, uint8_t bitrate, uint8_t *prbs_cmd, uint8_t prbs_cmd_len)
{
    ...
    NXPLOG_NCIHAL_D("phNxpNciHal_getPrbsCmd - prbs_type 0x%x hw_prbs_type = 0x%x", prbs_type, hw_prbs_type);
    if(NULL == prbs_cmd ||
#if(NFC_NXP_CHIP_PN548AD == 1)
            prbs_cmd_len != 0x09)
#else
