...
#include <string.h>
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
#include <math.h>
#endif
#include "gki.h"
...
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    if(LLCP_GetStart_Symm () == TRUE)
         llcp_cb.lcb.fw_symm_state = FALSE;
#endif
...
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
        if ((LLCP_GetStart_Symm () == TRUE))
        {   /*start llcp symmetry mechanism from firmware*/
            NFC_LlcpStartSymmetry ((llcp_cb.lcb.peer_lto * 0.1) ,ceil((llcp_link_rwt[llcp_cb.lcb.local_wt] * 0.1)));
        }
        else
            /* wait for data to receive from remote */
...
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    if((LLCP_GetStart_Symm () == TRUE) && llcp_cb.lcb.is_initiator)
        NFC_LlcpStopSymmetry();
#endif
...
static void llcp_link_send_SYMM (void)
{
    ...
    UINT8  *p;
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    if((LLCP_GetStart_Symm () == FALSE)||(llcp_cb.lcb.fw_symm_state == FALSE))
    {
#endif
...
void llcp_link_check_send_data (void)
{
    BT_HDR *p_pdu;
#if(NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    BOOLEAN fw_symmetry = LLCP_GetStart_Symm ();
#endif
    ...
    */
    ...
    if(
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
              (fw_symmetry == TRUE) || (llcp_cb.lcb.symm_state == LLCP_LINK_SYMM_LOCAL_XMIT_NEXT)
#else
              llcp_cb.lcb.symm_state == LLCP_LINK_SYMM_LOCAL_XMIT_NEXT
#endif
      )
    {
        ...
        */
        ...
        else
        {
            ...
            if (
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
                    (fw_symmetry == FALSE)&&(llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED)
#else
                    llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED
#endif
               )
...
static void llcp_link_proc_rx_data (BT_HDR *p_msg)
{
    ...
    if(
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
              (fw_symmetry == TRUE) || (llcp_cb.lcb.symm_state == LLCP_LINK_SYMM_REMOTE_XMIT_NEXT)
#else
              llcp_cb.lcb.symm_state == LLCP_LINK_SYMM_REMOTE_XMIT_NEXT
#endif
      )
    {
        ...
        if (
#if (NFC_NXP_OPEN_INCLUDED_NCI_1_1 == TRUE)
                (fw_symmetry == FALSE)&&(llcp_cb.lcb.received_first_packet == FALSE)
#else
                llcp_cb.lcb.received_first_packet == FALSE
#endif
            )
...
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
                        if((fw_symmetry == TRUE) && (llcp_cb.lcb.fw_symm_state == FALSE))
                        llcp_link_send_SYMM();
#endif
                    }
                    else
...
void llcp_link_connection_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    {
#if (NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
        if ((LLCP_GetStart_Symm () == FALSE) && (llcp_cb.lcb.symm_state == LLCP_LINK_SYMM_REMOTE_XMIT_NEXT))
        {
