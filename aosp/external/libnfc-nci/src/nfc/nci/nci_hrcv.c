...
void nci_proc_rf_management_rsp (BT_HDR *p_msg)
{
    ...
    case NCI_MSG_RF_PARAMETER_UPDATE:
        ...
        break;
#if(NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
    case NCI_MSG_RF_LLCP_SYMMETRY_START:
        //nfc_ncif_event_status (NFC_LLCP_SYMMETRY_START_REVT, *pp);
        if(*pp == NFC_STATUS_OK)
        {
             llcp_cb.lcb.fw_symm_state == TRUE;
             llcp_cb.lcb.received_first_packet = TRUE;
             LLCP_StartSymm_Rsp(LLCP_LINK_FIRST_PACKET_RECEIVED_EVT ,LLCP_LINK_SUCCESS);
        }
        else
             LLCP_StartSymm_Rsp(LLCP_LINK_ACTIVATION_FAILED_EVT ,LLCP_LINK_START_SYMM_FAILED);
        break;

    case NCI_MSG_RF_LLCP_SYMMETRY_STOP:
