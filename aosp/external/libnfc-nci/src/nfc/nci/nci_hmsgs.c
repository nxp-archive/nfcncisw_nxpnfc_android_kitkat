...
UINT8 nci_snd_core_init (void)
{
    ...
    UINT8_TO_STREAM (pp, NCI_VERSION);
...
/*******************************************************************************
**
** Function         nci_snd_llcp_start_symmetry_cmd
**
** Description      compose and send RF Management LLCP Start symmetry command
**                  to command queue
**
** Returns          status
**
*******************************************************************************/
UINT8  nci_snd_llcp_start_symmetry_cmd (UINT8 remote_lto,UINT8 local_lto)
{
     BT_HDR *p;
     UINT8  *pp;
     if ((p = NCI_GET_CMD_BUF (NCI_LLCP_PARAM_SIZE_START)) == NULL)
             return (NCI_STATUS_FAILED);
     p->event           = BT_EVT_TO_NFC_NCI;
     p->len             = NCI_MSG_HDR_SIZE + NCI_LLCP_PARAM_SIZE_START;
     p->offset          = NCI_MSG_OFFSET_SIZE;
     p->layer_specific  = 0;
     pp                 = (UINT8 *) (p + 1) + p->offset;

     NCI_MSG_BLD_HDR0 (pp, NCI_MT_CMD, NCI_GID_RF_MANAGE);
     NCI_MSG_BLD_HDR1 (pp, NCI_MSG_RF_LLCP_SYMMETRY_START);
     UINT8_TO_STREAM (pp, NCI_LLCP_PARAM_SIZE_START);
     UINT8_TO_STREAM (pp, remote_lto);
     UINT8_TO_STREAM (pp, local_lto);
...
/*******************************************************************************
**
** Function         nci_snd_llcp_stop_symmetry_cmd
**
** Description      compose and send RF Management LLCP Stop symmetry command
**                  to command queue
**
** Returns          status
**
*******************************************************************************/
UINT8  nci_snd_llcp_stop_symmetry_cmd(void)
{
     BT_HDR *p;
     UINT8  *pp;
     if ((p = NCI_GET_CMD_BUF (NCI_LLCP_PARAM_SIZE_STOP)) == NULL)
              return (NCI_STATUS_FAILED);
     p->event           = BT_EVT_TO_NFC_NCI;
     p->len             = NCI_MSG_HDR_SIZE + NCI_LLCP_PARAM_SIZE_STOP;
     p->offset          = NCI_MSG_OFFSET_SIZE;
     p->layer_specific  = 0;
     pp                 = (UINT8 *) (p + 1) + p->offset;

     NCI_MSG_BLD_HDR0 (pp, NCI_MT_CMD, NCI_GID_RF_MANAGE);
     NCI_MSG_BLD_HDR1 (pp, NCI_MSG_RF_LLCP_SYMMETRY_STOP);
     UINT8_TO_STREAM  (pp, NCI_LLCP_PARAM_SIZE_STOP);
