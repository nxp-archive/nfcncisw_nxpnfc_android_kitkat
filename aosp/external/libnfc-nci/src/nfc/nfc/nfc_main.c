...
static const tNCI_DISCOVER_MAPS nfc_interface_mapping[NFC_NUM_INTERFACE_MAP] =
{
    ...
    ,
    /* This mapping is for LLCP Low Rf Interface  */
    {
        NCI_PROTOCOL_NFC_DEP,
        NCI_INTERFACE_MODE_POLL_N_LISTEN,
        NCI_INTERFACE_LLCP_LOW
    }
#else
    ,
...
#if(NFC_NXP_NOT_OPEN_INCLUDED_NCI_1_1 == TRUE)
/*******************************************************************************
 **Function         NFC_LlcpStartSymmetry
 **
 **Description      this function is called to start llcp symmetry mechanism
 **
 **Parameters       remote_lto - peer link timeout
 **                 local_lto - local link timeout
 **
 **Returns          void
 *******************************************************************************/
tNFC_STATUS NFC_LlcpStartSymmetry(UINT8 remote_lto,
                                  UINT8 local_lto)
{
    return nci_snd_llcp_start_symmetry_cmd(remote_lto, local_lto);
}
/*******************************************************************************
 **Function         NFC_LlcpStopSymmetry
 **
 **Description      this function is called to stop lllp symmetry mechanism
 **
 **Returns          void
 *******************************************************************************/
tNFC_STATUS NFC_LlcpStopSymmetry(void)
{
    return nci_snd_llcp_stop_symmetry_cmd();
}
#endif
/*******************************************************************************
**
