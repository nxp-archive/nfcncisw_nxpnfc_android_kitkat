    ...
    if (evt_data.activate.intf_param.type == NCI_INTERFACE_ISO_DEP)
    {
        ...
        case NCI_DISCOVERY_TYPE_LISTEN_B:
            ...
            */
            ...
            ||(evt_data.activate.intf_param.type == NCI_INTERFACE_LLCP_LOW)
#endif
    )
    {
        ...
        while ((p_evt = (BT_HDR *)GKI_getfirst (&p_cb->rx_q)) != NULL)
        {
            ...
            if (p_cb->conn_id == NFC_RF_CONN_ID)
            {
                ...
                if ((p_cb->act_protocol >= NCI_PROTOCOL_T1T) && (p_cb->act_protocol <= NCI_PROTOCOL_T3T))
                {
                    ...
                    {
                        UINT8 Ack = *(p + p_evt->offset + p_evt->len-1);;
                        UINT8 valid_bits = ((*(p + p_evt->offset + p_evt->len)) & MAX_NUM_VALID_BITS_FOR_ACK);
                        NCI_CALCULATE_ACK(Ack, valid_bits);
                        data_cevt.status = (Ack == T1T_WRITE_RSP_ACK)?NFC_STATUS_OK:NFC_STATUS_FAILED;
                    }
                    else
#endif
