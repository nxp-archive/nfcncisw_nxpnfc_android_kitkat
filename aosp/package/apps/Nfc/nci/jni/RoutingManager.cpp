...
void *reader_req_event_ntf_handler_async(void* arg);
void reader_req_event_ntf_timoutHandler (union sigval);
RoutingManager::~RoutingManager ()
{
...
bool RoutingManager::initialize (nfc_jni_native_data* native)
{
  ...
  //  mIsDirty = true;
    ...
    swp_rdr_req_ntf_info.swp_rd_state = STATE_SE_RDR_MODE_INVALID;
    ...
    case NFA_EE_DISCOVER_REQ_EVT:
        ...
        for (UINT8 xx = 0; xx < info.num_ee; xx++)
        {
            ...
            if(info.ee_disc_info[xx].pa_protocol ==  0x04 || info.ee_disc_info[xx].pb_protocol == 0x04)
            {
                ...
                swp_rdr_req_ntf_info.swp_rd_req_timer.kill();
                swp_rdr_req_ntf_info.swp_rd_req_timer.set (rdr_req_handling_timeout, reader_req_event_ntf_timoutHandler);
                swp_rdr_req_ntf_info.mMutex.unlock();
...
void *reader_req_event_ntf_handler_async(void *arg)
{
    ...
    static const char fn [] = "RoutingManager::reader_req_event_ntf_handler_async";
    ALOGD ("%s:  ", fn);
    swp_rdr_req_ntf_info.mMutex.lock();
    //case1: first time
    //case2:handle only if all the previously added technologies are removed
    if((swp_rdr_req_ntf_info.swp_rd_req_current_info.src == NFA_HANDLE_INVALID ) &&
       ((swp_rdr_req_ntf_info.swp_rd_req_info.tech_mask & NFA_TECHNOLOGY_MASK_A) ||
        (swp_rdr_req_ntf_info.swp_rd_req_info.tech_mask & NFA_TECHNOLOGY_MASK_B)))
    {
        ...
        SecureElement::getInstance().handleEEReaderEvent(NFA_RD_SWP_READER_REQUESTED,
                                                         swp_rdr_req_ntf_info.swp_rd_req_info.tech_mask,
                                                         swp_rdr_req_ntf_info.swp_rd_req_info.src);
         swp_rdr_req_ntf_info.mMutex.unlock();
    }
    else if((!swp_rdr_req_ntf_info.swp_rd_req_info.tech_mask) &&
            (swp_rdr_req_ntf_info.swp_rd_req_current_info.src == swp_rdr_req_ntf_info.swp_rd_req_info.src))
    {

        SecureElement::getInstance().handleEEReaderEvent(NFA_RD_SWP_READER_STOP,
                                                     swp_rdr_req_ntf_info.swp_rd_req_info.tech_mask,
                                                     swp_rdr_req_ntf_info.swp_rd_req_info.src);
        swp_rdr_req_ntf_info.swp_rd_req_current_info.tech_mask &= ~NFA_TECHNOLOGY_MASK_A;
        swp_rdr_req_ntf_info.swp_rd_req_current_info.tech_mask &= ~NFA_TECHNOLOGY_MASK_B;
    ...
    #if 0 //un handled requests
        if(swp_rdr_req_ntf_info.swp_rd_state == STATE_SE_RDR_MODE_STARTED)
            swp_rdr_req_ntf_info.swp_rd_req_fail_cause = STATE_SE_RDR_FAILURE_NOT_ALLOWED;

        else
            swp_rdr_req_ntf_info.swp_rd_req_fail_cause = STATE_SE_RDR_FAILURE_NOT_SUPPORTED;

        SecureElement::getInstance().handleEEReaderEvent(NFA_RD_SWP_READER_START_FAIL,
                                                         swp_rdr_req_ntf_info.swp_rd_req_fail_cause,
                                                         swp_rdr_req_ntf_info.swp_rd_req_info.src);
    #endif
...
void reader_req_event_ntf_timoutHandler (union sigval)
{
    ...
    static const char fn [] = "RoutingManager::reader_req_event_ntf_timoutHandler";
    ALOGD ("%s:  ", fn);
    pthread_t thread;
    pthread_create (&thread, NULL,  &reader_req_event_ntf_handler_async,NULL);
}
